...
function InputHeader({
  ...
  return (
    <div
      ...
      style={{ backgroundColor: color, position: "sticky", top: 0 }}
    >
      {/* color indicator / pre-enlisted heading */}
      {isPreEnlisted ? (
        <h5
          style={{ ...flStyle, marginTop: 0, marginBottom: 0, color: "white" }}
        >
          Pre-Enlisted Classes
        </h5>
      ) : (
      ...
      <IconButton
        ...
        text="Paste from AISIS"
        onClick={onCopyClick}
      />
      {showDelete && !isPreEnlisted && (
        <IconButton
          cStyle={mrgLeft}
          icon={<FaTrashAlt />}
          text="Delete Group"
